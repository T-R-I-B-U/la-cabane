...
Context file for Gemini CLI. Read this at the start of every session.
...
- Commits suggested by Gemini CLI must be in English, following the conventional
...
- **Always run `npm run lint` and `npx prettier --check "src/**/*.{js,jsx,css,json}"` before committing changes.**
  Fix every error and formatting issue before finalizing changes — neither lint errors
