import { useEffect, useMemo, useRef } from 'react'
import { useFrame, useThree } from '@react-three/fiber'
import * as THREE from 'three'
import { useStableInteractionCallback } from '../interactions/useStableInteractionCallback'

const CENTER_NDC = new THREE.Vector2(0, 0)
const HOVER_EMISSIVE = new THREE.Color(0xffefbf)
const HOVER_EMISSIVE_INTENSITY = 0.35

function findLadderMeshes(cabane) {
  if (!cabane) return []
  const found = []
  cabane.traverse((obj) => {
    if ((obj.isMesh || obj.isInstancedMesh) && obj.name === 'ladder') found.push(obj)
  })
  return found
}

function forEachMaterial(material, callback) {
  if (Array.isArray(material)) material.forEach(callback)
  else if (material) callback(material)
}

function cloneMaterial(material) {
  return Array.isArray(material) ? material.map((m) => m.clone()) : material.clone()
}

export function ClickableLadder({ cabane, active, onInteract }) {
  const { camera } = useThree()
  const hoveredRef = useRef(false)
  const materialStatesRef = useRef(new Map())
  const raycaster = useRef(new THREE.Raycaster())
  const onInteractRef = useStableInteractionCallback(onInteract)

  const ladderMeshes = useMemo(() => {
    const found = findLadderMeshes(cabane)
    found.forEach((mesh) => {
      if (mesh.material) mesh.material = cloneMaterial(mesh.material)
    })
    return found
  }, [cabane])

  // Override / restore raycast based on active state
  useEffect(() => {
    if (!ladderMeshes.length) return
    if (active) {
      // Remove the own-property no-op set by instancing.js, restoring prototype raycast
      ladderMeshes.forEach((m) => {
        delete m.raycast
      })
    } else {
      ladderMeshes.forEach((m) => {
        m.raycast = () => {}
      })
    }
    return () => {
      ladderMeshes.forEach((m) => {
        m.raycast = () => {}
      })
    }
  }, [active, ladderMeshes])

  useEffect(() => {
    materialStatesRef.current = new Map()
    ladderMeshes.forEach((mesh) => {
      forEachMaterial(mesh.material, (material) => {
        if (!material.emissive) return
        materialStatesRef.current.set(material, {
          emissive: material.emissive.clone(),
          emissiveIntensity: material.emissiveIntensity ?? 0,
        })
      })
    })
  }, [ladderMeshes])

  useEffect(() => {
    const onPointerDown = (event) => {
      if (event.button !== 0 || !hoveredRef.current) return
      onInteractRef.current?.()
    }
    document.addEventListener('pointerdown', onPointerDown)
    return () => document.removeEventListener('pointerdown', onPointerDown)
  }, [onInteractRef])

  useFrame(() => {
    if (!active || !ladderMeshes.length) {
      ladderMeshes.forEach((mesh) => {
        forEachMaterial(mesh.material, (material) => {
          const original = materialStatesRef.current.get(material)
          if (!original || !material.emissive) return
          material.emissive.copy(original.emissive)
          material.emissiveIntensity = original.emissiveIntensity
        })
      })
      hoveredRef.current = false
      return
    }

    raycaster.current.setFromCamera(CENTER_NDC, camera)
    const hits = raycaster.current.intersectObjects(ladderMeshes, true)
    const isHovered = hits.length > 0
    hoveredRef.current = isHovered

    ladderMeshes.forEach((mesh) => {
      forEachMaterial(mesh.material, (material) => {
        const original = materialStatesRef.current.get(material)
        if (!original || !material.emissive) return
        if (isHovered) {
          material.emissive.copy(HOVER_EMISSIVE)
          material.emissiveIntensity = HOVER_EMISSIVE_INTENSITY
        } else {
          material.emissive.copy(original.emissive)
          material.emissiveIntensity = original.emissiveIntensity
        }
      })
    })
  })

  return null
}
