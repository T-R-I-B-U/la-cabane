...
function findReceptionMeshes(cabane) {
  if (!cabane) return []

  const parent = cabane.getObjectByName(RECEPTION_PARENT)
  if (!parent) return []
...
export function ClickableReception({ cabane, active, onInteract }) {
  ...
  const receptionMeshes = useMemo(() => {
    const found = findReceptionMeshes(cabane)
    found.forEach((mesh) => {
  ...
  useFrame(() => {
    if (!active || !receptionMeshes.length) {
      receptionMeshes.forEach((mesh) => {
        forEachMaterial(mesh.material, (material) => {
    ...
    const hits = raycaster.current.intersectObjects(receptionMeshes, true)
